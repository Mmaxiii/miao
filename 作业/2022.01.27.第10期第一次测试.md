...
jpge(jpg / jpe):有损压缩，对色彩保留的较好，适合存图片
...
```
描述复杂度的一个符号，他描述一个函数的增长率不超过某个函数
具体到某个算法中，是描述一段程序的运行时间随着输入的提升所发生的的变化不会超过某个函数的增长速度
```
95. for in循环与for of循环的区别是什么？for in循环有什么需要注意的问题？in运算符有什么需要注意的问题？
```
for in 是遍历键
for of 是遍历值
for in 会遍历出原型链上所有可遍历属性，要给属性加enumerable:false 使其不可遍历
in 原型链上的属性也会返回true，可以通过obj.hasOwnProperty()判断自有属性
```
